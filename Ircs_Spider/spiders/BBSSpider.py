...
class BBSSpider(CrawlSpider):
    ...
    def start_requests(self):
        ...
        for hd in newzspt_bbs_hds:
            hd_id = hd[0]
            hd_url = hd[1]
            stockcode = hd[2]
            rid_pattern = re.findall("=\d+", hd_url)[0]
            boardid = rid_pattern[1:len(rid_pattern)]
            newzspt_bbs_url = "http://newzspt.p5w.net/bbs/question_page.asp?boardid=%s&bbs=1&pageNo=1" % boardid
            yield FormRequest(url=newzspt_bbs_url,
                              meta={"hd_id": hd_id, "stockcode": stockcode, "pageNo": '1', "boardid": boardid},
